	...
	server.tool(
		{
			name: 'web_extract',
			description:
				'Extract, process, or summarize web content from URLs. Use when you need to read page content, summarize articles, crawl sites, or extract structured data. Providers: tavily (content extraction), kagi (summarization of pages/videos/podcasts), firecrawl (scraping/crawling/mapping/structured extraction/interactive), exa (content retrieval/similar pages), keenable (fetch; keyed or opt-in public).',
			annotations: {
				readOnlyHint: true,
			...
			schema: v.object({
				...
				mode: v.optional(
					v.pipe(
						...
						v.description(
							'Processing mode. Firecrawl: scrape/crawl/map/extract/actions. Exa: contents/similar. Tavily/Keenable: extract. Kagi: summarize. Defaults to provider default.',
						),
					),
				),
