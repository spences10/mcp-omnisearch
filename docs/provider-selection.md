...
| `parallel`        | `PARALLEL_API_KEY` | Long LLM-ready excerpts; explicit-only because pricing can surprise   | Translates `site:` / `-site:` and `include_domains` / `exclude_domains` into Parallel `source_policy`. Uses `mode: "basic"` to keep cost down. |
...
| `parallel`  | `PARALLEL_API_KEY`  | `extract`                                      | Long excerpts; `extract_depth: "advanced"` requests full Markdown.         |
...
- Need semantic discovery, similar pages, or meaning-based results?
  Use `exa`.
- Need long source excerpts and can accept Parallel pricing? Use
  `parallel` explicitly.
