...
| `querit`          | `QUERIT_API_KEY` | Multilingual and real-time search                                     | Translates `site:` / `-site:`, `lang:`, `loc:`, and `before:` / `after:` into Querit `filters` (`sites`, `languages`, `geo`, `timeRange`). Search only. |
...
- Need semantic discovery, similar pages, or meaning-based results?
  Use `exa`.
- Need multilingual or real-time international results? Use `querit`.
