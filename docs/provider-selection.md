...
| `keenable`        | `KEENABLE_API_KEY` (or `KEENABLE_ALLOW_PUBLIC=1`) | Independent web index; lowest-priority explicit fallback              | A single `site:` / `include_domains` value becomes `site`. `after:` / `before:` map to `published_after` / `published_before`. Public tier is opt-in, shared, and has no SLA. |
...
| `keenable`  | `KEENABLE_API_KEY` (or `KEENABLE_ALLOW_PUBLIC=1`) | `extract`                                      | Indexed fetch; `extract_depth: "advanced"` adds `live=true`.               |
...
- Need semantic discovery, similar pages, or meaning-based results?
  Use `exa`.
- Need an independent index or an opt-in public fallback? Use
  `keenable` explicitly.
